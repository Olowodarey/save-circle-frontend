...
export function useGroupDetails(groupId: string) {
  ...
  const [groupDetails, setGroupDetails] = useState<FormattedGroupDetails | null>(null)
  ...
  const formatGroupDetails = (groupInfo: GroupInfo, groupId: string): FormattedGroupDetails => {
    // Convert cycle unit enum to readable frequency
    const getFrequency = (unit: any, duration: any) => {
      const unitNum = typeof unit === 'object' && unit.variant ? Object.keys(unit.variant)[0] : Number(unit)
      ...
      if (typeof unit === 'object' && unit.variant) {
        ...
        if (variantKey === 'Days') {
          ...
          return durationNum === 1 ? "Weekly" : durationNum === 2 ? "Bi-weekly" : `Every ${durationNum} weeks`
    ...
    const generateTags = (groupInfo: GroupInfo) => {
      ...
      const visibilityValue = typeof groupInfo.visibility === 'object' && groupInfo.visibility.variant 
        ? Object.keys(groupInfo.visibility.variant)[0] 
        : Number(groupInfo.visibility)
      
      if (visibilityValue === 0 || visibilityValue === 'Public') tags.push("public")
      if (visibilityValue === 1 || visibilityValue === 'Private') tags.push("private")
      
      // Add frequency-based tags
      const cycleUnit = typeof groupInfo.cycle_unit === 'object' && groupInfo.cycle_unit.variant
        ? Object.keys(groupInfo.cycle_unit.variant)[0]
        : Number(groupInfo.cycle_unit)
      const cycleDuration = Number(groupInfo.cycle_duration)
      
      if ((cycleUnit === 1 || cycleUnit === 'Weeks') && cycleDuration === 1) tags.push("weekly")
      if ((cycleUnit === 1 || cycleUnit === 'Weeks') && cycleDuration === 2) tags.push("bi-weekly")
    ...
    return {
      ...
      description: `A ${isPublic ? 'public' : 'private'} savings group with ${maxMembers} members contributing ${formatContribution(contributionAmount)} ${getFrequency(groupInfo.cycle_unit, groupInfo.cycle_duration).toLowerCase()}.`,
      ...
      status: state,
      createdAt: new Date(Number(groupInfo.start_time) * 1000).toISOString().split('T')[0],
      nextPayoutDate: nextPayoutDate.toISOString().split('T')[0],
      currentCycle,
      totalCycles,
      totalPoolAmount,
      isUserMember: false, // Will be determined when fetching members
      userCanJoin: memberCount < maxMembers && state === 'active' && meetsReputationRequirement,
      userReputation, // Add user reputation to the returned object
      contributionAmount: typeof groupInfo.contribution_amount === 'bigint' ? groupInfo.contribution_amount : BigInt(groupInfo.contribution_amount),
      state,
  ...
  const formatMember = (member: GroupMember, index: number, isCreator: boolean = false): FormattedMember => {
  ...
  const fetchGroupDetails = async () => {
    ...
    try {
      ...
      for (let i = 0; i < memberCount; i++) {
        memberPromises.push(contract.call("get_group_member", [{ low: groupId, high: 0 }, i]) as Promise<GroupMember>)
      }
