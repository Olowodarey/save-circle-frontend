...
export function useGroups() {
  ...
  const formatGroup = (groupInfo: GroupInfo, groupId: string): FormattedGroup => {
    ...
    return {
      ...
      description: `A ${isPublic ? 'public' : 'private'} savings group with ${Number(groupInfo.member_limit)} members contributing ${formatContribution(groupInfo.contribution_amount)} ${getFrequency(groupInfo.cycle_unit, groupInfo.cycle_duration).toLowerCase()}.`,
  ...
  const fetchGroups = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= maxGroupsToCheck; i++) {
        try {
          const groupInfo = await contract.call("get_group_info", [i]) as GroupInfo
