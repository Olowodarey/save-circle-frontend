...
export function useLiquidityLock() {
  ...
  const fetchLockedBalance = async () => {
    ...
    try {
      const balance = await contract.call("get_locked_balance", [address]) as bigint
  ...
  const lockLiquidity = async (groupId: string, amount: string, tokenAddress: string = CONTRACT_ADDRESS) => {
